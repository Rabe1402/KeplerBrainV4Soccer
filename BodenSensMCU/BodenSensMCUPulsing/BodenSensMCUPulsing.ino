// *******************************************************************************
// ***  KeplerBRAIN V4 Sensor STAMP SPI Client Soccer Bodensensor V 07-05-2026 by Korbinian G. ***
// *******************************************************************************
...
void setup()
{
  ...
  Serial.begin(115200);
  delay(100); // ADC/DMA Zeit zum Stabilisieren
  measure_baseline(); // <-- hier einmalig messen
}
// BodenSensMCU - Pulse Counting via synchronem ADC
// PA15 = IR LED Transistor (schon vorhanden)

const int TOTAL_PULSES = 500;        // Pulse pro Messung
const int PULSE_ON_US  = 20;         // µs LED an  (~38kHz)
const int PULSE_OFF_US = 20;         // µs LED aus
const int THRESHOLD = 1900;         // gerätschaft


// Baseline (LED komplett aus gemessen)
uint32_t baseline[8] = {0};

// Ergebnisse
uint32_t pulse_counts[8] = {0};

void measure_baseline()
{
  digitalWrite(PA15, HIGH); // LED aus
  delay(10);                // kurz warten bis stabil
  
  // Durchschnitt über 50 Messungen
  uint32_t sum[8] = {0};
  for (int i = 0; i < 50; i++)
  {
    delayMicroseconds(100);
    for (int s = 0; s < 8; s++)
      sum[s] += adcValues[s]; // DMA füllt adcValues[] laufend
  }
  for (int s = 0; s < 8; s++)
    baseline[s] = sum[s] / 50;
...
void pulse_count_sensors()
{
  for (int s = 0; s < 8; s++) pulse_counts[s] = 0;

  for (int p = 0; p < TOTAL_PULSES; p++)
  {
    digitalWrite(PA15, HIGH); // LED AN
    ...
    for (int s = 0; s < 8; s++)
    {
      ...
      Serial.print(s);
      Serial.print(":");
      Serial.print(adcValues[s]);
      Serial.print("\t");
      if (adcValues[s] > baseline[s] + THRESHOLD)
        pulse_counts[s]++;
    }
    Serial.println();

    digitalWrite(PA15, HIGH); // LED AUS
    delayMicroseconds(PULSE_OFF_US);
  }

  // NUR beim ersten Mal alle Rohwerte ausgeben
  static bool printed = false;
  if (!printed)
  {
    Serial.println("=== DEBUG ===");
    Serial.print("Baseline:  ");
    for (int s = 0; s < 8; s++) { Serial.print(baseline[s]); Serial.print(" "); }
    Serial.println();
    
    // Einmal LED an und direkt lesen
    digitalWrite(PA15, LOW);
    delayMicroseconds(PULSE_ON_US);
    Serial.print("LED-ON:    ");
    for (int s = 0; s < 8; s++) { Serial.print(adcValues[s]); Serial.print(" "); }
    Serial.println();
...
void loop()
{
  // Baseline einmal am Anfang, dann alle 5 Sek neu
  static unsigned long last_baseline = 0;
  if (millis() - last_baseline > 5000)
  {
    measure_baseline();
  ...
  // Counts in spi_data packen (statt raw ADC)
  // Format bleibt gleich: 2 Bytes pro Sensor
  for (int i = 0; i < 8; i++)
  {
    spi_data[1 + i * 2]     = (pulse_counts[i] >> 8) & 0xFF;
    spi_data[1 + i * 2 + 1] = pulse_counts[i] & 0xFF;
  }

  // Debug
  /*
  if (millis() - last_baseline > 500)
  {  
  Serial.print("F:");  Serial.print(pulse_counts[0]);
  Serial.print(" FL:"); Serial.print(pulse_counts[1]);
  Serial.print(" FR:"); Serial.print(pulse_counts[2]);
  Serial.print(" L:");  Serial.print(pulse_counts[3]);
  Serial.print(" R:");  Serial.print(pulse_counts[4]);
  Serial.print(" BL:"); Serial.print(pulse_counts[5]);
  Serial.print(" BR:"); Serial.print(pulse_counts[6]);
  Serial.print(" B:");  Serial.println(pulse_counts[7]); 
  }
  */
}
